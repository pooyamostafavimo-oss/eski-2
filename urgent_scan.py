...
Extra features:
- Voice messages (Telegram voice notes) are skipped entirely — they are
  never classified and never posted to the target channel.
...
# Skip Telegram voice notes entirely (never classify, never post them).
# Set SKIP_VOICE_MESSAGES=0 in the env if you ever want them back.
SKIP_VOICE_MESSAGES = os.environ.get("SKIP_VOICE_MESSAGES", "1").strip().lower() not in (
    "0", "false", "no", "off",
)
...
def is_voice_message(msg) -> bool:
    """True for Telegram voice notes (and, to be safe, round video notes).

    `msg.voice` is set by Telethon only for actual voice messages, not for
    regular audio files or videos, so normal media still goes through.
    """
...
async def post_group(client: TelegramClient, group: list, final_text: str) -> None:
    ...
    # Extra safety net: even if a voice note somehow made it this far,
    # never attach it to a post.
    media_list = [
        m.media for m in group
        if m.media and not (SKIP_VOICE_MESSAGES and is_voice_message(m))
    ]
...
async def process_message_group(
    ...
    try:
        ...
        # fast-tracking a message and the full hourly run reprocessing
    ...
    for channel in SOURCE_CHANNELS:
        ...
        try:
            async for msg in client.iter_messages(
                ...
                # Voice notes are dropped before anything else: they're
                # never classified, never posted. We still advance
                # newest_seen so we don't re-read them next run.
                if SKIP_VOICE_MESSAGES and is_voice_message(msg):
                    newest_seen = max(newest_seen, msg.id)
                    print(f"[SKIP] Voice message {msg.id} from {channel}")
                    continue
